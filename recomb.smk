    ...
    run:
        with open(output.aneuploidy_tsv, "w") as out:
              ...
              for c in chroms:
                  ...
                  out.write(
                      f"{data['mother_id']}\t{data['father_id']}\t{data['child_id']}\t{c}\t{data['sigma_baf']}\t{data['pi0_baf']}\t{dat  a['0']}\t{data['1m']}\t{data['1p']}\t{data['2']}\t{data['3m']}\t{data['3p']}\t{max_bf}\t{max_cat}\t{data['prop01_exp_het']}\n"
                  )


rule mendelian_filtering:
    """Filtering to mendelian violating variants."""
    input:
        ...
        aneuploidy_calls = lambda wildcards: expand("results/aneuploidy_calls/{{family}}/{embryo}.aneuploidy.tsv", embryo=config["families"][wildcards.family]["embryos"].keys())
    output:
